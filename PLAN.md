...
| Post-launch — public MCP server (AWS showcase) | 🚧 Built 2026-08-08, awaiting bootstrap + gated go-live: read-only MCP server (`mcp/`, 3 tools over public forecast data) on Lambda + auth-gated API Gateway, single Terraform stack (`infra/`), deploys via OIDC only (no long-lived AWS keys), `infra/BOOTSTRAP.md` one-sitting account runbook, security posture in `mcp/README.md`. |
