...
| Post-launch — aggregate-only data layer (security-definer RPCs) | 🚧 PR 1/2 built 2026-08-08: migration 0013 flips `brisbane_daily_avg_u91` to `SECURITY DEFINER` (+ pinned `search_path` + 366-day range guard) and adds three definer RPCs (`latest_snapshot_ingested_at`, `snapshot_event_bound`, `live_coverage_ramp_end`) so no anon read path touches `sites`/`price_snapshots` directly; app reads switched over. PR 2/2 (after PR 1 deploys): revoke anon SELECT + drop anon RLS policies on both tables, closing the latent per-station exposure left from the Section-2 cut (migrations 0001/0004 predate it; verified not client-exposed today — anon key is server-side only). Also lands two parked audit items (SQL `liveCoverageRampEnd`, `brisbane_daily_avg_u91` hardening). |
...
- ~~**`liveCoverageRampEnd` rewrite to SQL aggregation.**~~ ✅ Landed 2026-08-08 via migration 0013 (`live_coverage_ramp_end` security-definer RPC — one date out, semantics preserved; caching was already in place via `getCachedCoverageDeadzones`). Note: the "backdated `transaction_date_utc` can trigger an early threshold-cross" caveat is inherent to the first-seen semantics and carries over — revisit if it ever bites.
- ~~**`brisbane_daily_avg_u91` hardening.**~~ ✅ Landed 2026-08-08 via migration 0013 (`SET search_path = public, pg_temp` + 366-day range guard; also flipped to `SECURITY DEFINER` as part of the aggregate-only data-layer work).
