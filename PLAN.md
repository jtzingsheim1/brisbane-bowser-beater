...
| Post-launch — MCP docs Q&A (Bedrock RAG) | ✅ Live 2026-08-22, verified end-to-end same day as the design commit (design in `docs/mcp-rag-design.md`, committed before implementation; feature PR #98, deploy fixes #99 + #100): two new MCP tools — `search_docs` (Bedrock Retrieve) and `ask_docs` (RetrieveAndGenerate, Claude Haiku 4.5 via the `au.` inference profile) — over a curated allowlist of this repo's public docs (`mcp/corpus-manifest.txt`; internal notes excluded, enforced by test), on a Bedrock Knowledge Base + S3 Vectors index (Titan V2 embeddings), all in the existing Terraform stack. Layered AWS-enforced cost guards: gateway monthly quota cut 100k→500 **per key** (so 1000 across the two keys, the second added in #104; the ceiling scales with key count), per-request caps, and a Terraform-managed USD 5 budget action that attaches `Deny bedrock:*` to the server role at 100% (confirmed armed/Standby). Live E2E 4/4: tools/list shows 5 tools, search_docs returns cited chunks, ask_docs returns a grounded cited answer, unauthenticated calls get 403. Deploy surfaced two IAM findings now baked into the code/docs: `bedrock:GetInferenceProfile` is required for profile-routed generation (#100), and role descriptions must stay static under a least-privilege deploy role (#99). Residual hardening landed via issue #101 (closed 2026-08-23; see the MCP/infra hardening row below). |
| Post-launch — MCP/infra hardening (issue #101) | ✅ Closed 2026-08-23 (PRs #102, #106, #107). Each role Terraform creates carries a permissions boundary the deploy role cannot create, edit, remove or swap, hand-created in the bootstrap so the pipeline cannot widen its own ceiling. `tests/infra-boundaries.test.ts` holds the boundary documents and the roles' inline Terraform policies to exact action parity in both directions (actions only, not resource scoping), pins each role's grants individually, and checks the deploy policy's boundary pinning and self-modification Deny — verified by eighteen deliberate weakenings, each caught by a named test. The one deferred item (narrowing the workload boundary's Logs resource to the single granted log group) rode the corpus-sync bootstrap update rather than costing an operator step of its own. **Lambda reserved concurrency was considered and dropped**, not deferred: bounding parallelism does not bound spend when the usage plan already caps the month by request count, and the account runs one function (reasoning in `infra/variables.tf` and `docs/mcp-rag-design.md`). |
| Post-launch — ungated docs-corpus publishing | ✅ Live 2026-08-24 (PR #108), verified end to end the same day: doc merges to main re-sync the RAG knowledge base automatically via `.github/workflows/corpus-sync.yml`, under a narrow hand-created `bbb-mcp-corpus-sync` role that can write the corpus bucket and start ingestion and nothing else (`infra/BOOTSTRAP.md` section 3c). Ends the manual deploy every docs PR used to need; `mcp-deploy.yml` stays approval-gated for code and infra, and still syncs after each apply. Its trust policy pins the main ref rather than an event, which AWS cannot narrow further (web identity federation matches aud/azp/amr/sub only), so `tests/corpus-sync-workflow.test.ts` pins which workflows may hold `id-token: write` and rejects `pull_request_target`. Whether code/infra deploys could also run ungated behind machine-checkable CI controls was considered and **dropped** — the doc toil that motivated it is gone and the gate costs nothing. |
| Post-launch — public-surface language pass | ✅ Landed 2026-08-25 (PR #110), prompted by a cold outside read-through of the public repo. Corpus docs stopped naming who is assumed to read them (`docs/cycle-visuals-design.md` + one line of `docs/mcp-rag-design.md` reworded to generic technical readers; the design doc's audience *decision* stays, since it drives real design choices); the guard is a second term list, `AUDIENCE_ANCHORED_TERMS` in `mcp/src/banned-language.ts`, swept over every corpus doc by `mcp/src/manifest.test.ts` (mutation-verified) so the framing cannot silently return. Public docs also stopped restating tunable values as live fact: README presented the 60-minute staleness *default* as the live behaviour (the live threshold is whatever `BBB_STALENESS_MINUTES` is set to in Vercel), the Bedrock budget amount now reads as the mechanism (`budget_limit_usd`) rather than a dollar figure, and quota arithmetic is stated per key instead of as aggregates that rot when a key is added. PR #104's description was reworded in place (merged commits untouched). |
...
**Triaged 2026-08-23** (hardening burst): every item was re-verified against the then-current code. The bulk landed in the burst PR (timing-safe `CRON_SECRET` via `src/lib/cron-auth.ts`, freshness/rate-limit/cron-auth test matrices, `@vitest/coverage-v8`, Supabase client singletons, cached-plan replay notice, stable `AgentChat` keys, immutable `mergeSpans`, `forecasts` covering index + `visitor_hash` CHECK migration, retention pruning in `scripts/generate-forecast.ts` — the job the daily GitHub Actions schedule actually runs, *not* the unscheduled `/api/cron/forecast` route, which stays write-only because it is open when `CRON_SECRET` is unset) — see that PR's description for the mapping. Four items were deleted as stale: the postcss Dependabot alert (alerts tab confirmed clear after PR #97), the `getClientIp` spoofability comment (present in `usage.ts` for a while), and the two `backfill-csv.mjs` refactors (one-shot script, backfills long complete). The plan-cache `hashSituation` scope narrowing was closed by documenting it in a code comment (the audit's option B).

**Closed out 2026-08-24.** The second test batch landed (agent-route input validation, `getClientIp` precedence, `forecast/params` validation), alongside a guard over the security headers themselves, which nothing previously covered. The **CSP header** landed as a deliberate subset: `base-uri`, `object-src`, `frame-ancestors` and `form-action`, which need no nonce and close real attack classes. `script-src` was **considered and left unset** rather than parked again — a useful one needs per-request nonces, which means a `proxy.ts` minting one on every HTML request, and the fallback `'unsafe-inline'` permits exactly what a CSP exists to stop. There is no XSS surface today to justify that; the reasoning and its revisit trigger are in `next.config.ts`.
...
**Known caveat, not a task:** backdated `transaction_date_utc` can trigger an early `liveCoverageRampEnd` threshold-cross. Inherent to first-seen semantics, accepted — recorded so it is recognised if it ever surfaces, not tracked as work.
