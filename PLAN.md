...
| Phase 2 — Data ingestion + forecast model | 🚧 In progress (schema, CSV backfill, `brisbane_daily_avg_u91` aggregate, cycle characterisation → `cycle_params.json`, the TS daily projection → `forecasts` table, chunk-5 figure adoption, and the **live 30-min GitHub Actions ingestion** all landed — the chart now runs on current data; remaining: scheduling the daily forecast/narrative regeneration) |
...
- ✅ **30-min cron via GitHub Actions** (free for public repos — keeps Vercel Hobby tier clean and satisfies LUL 2.3) — **landed**:
  ...
  - ⏳ Daily `forecasts` / `daily_narrative` regeneration not yet scheduled — the projection currently runs only via the Vercel route `/api/cron/forecast`; needs either a Vercel cron (Phase 8) or a sibling GitHub Actions job.
...
8. **Schedule daily forecast + narrative regeneration.** Now that live history flows, the forecast should be regenerated daily off it (and the daily narrative line generated). The projection logic exists (`/api/cron/forecast`); it needs a schedule — a sibling GitHub Actions job mirroring the ingest pattern, or a Vercel cron at Phase 8.
