...
**Closed out 2026-08-24.** The second test batch landed (agent-route input validation, `getClientIp` precedence, `forecast/params` validation -- 50 tests, each cap mutation-verified). The **CSP header** landed as a deliberate subset: `base-uri`, `object-src`, `frame-ancestors` and `form-action`, which need no nonce and close real attack classes. `script-src` was **considered and left unset** rather than parked again -- a useful one needs per-request nonces, which means middleware on every request and opting every page out of static rendering, and the fallback `'unsafe-inline'` permits exactly what a CSP exists to stop. There is no XSS surface today to justify that cost; the reasoning and its revisit trigger are in `next.config.ts`. What remains below is not code work.
