...
- **MCP/infra hardening** — was tracked in **issue #101**, closed 2026-08-23 (landed via PRs #102, #106, #107), with one item deferred to the next CloudShell re-paste, which the 2026-08-24 corpus-sync bootstrap update carries. The permissions-boundary change merged and was applied on 2026-08-23 (Step 3 CloudShell paste, then deploy run 8 with a successful `Terraform apply`): each role Terraform creates carries a ceiling the deploy role cannot create, edit, remove, or swap, and the boundaries are created by hand in the bootstrap so the pipeline cannot widen its own limit. `tests/infra-boundaries.test.ts` holds the boundary documents and the roles' inline Terraform policies to exact action parity in both directions (actions only, not resource scoping), pins each role's grants individually, and checks the deploy policy's boundary pinning and self-modification Deny. Verified by mutation: eighteen deliberate weakenings of `infra/`, each caught by a named test, plus two checks that a *stronger* policy still passes. That deferred item was narrowing the workload boundary's Logs resource to the single granted log group; it rode the corpus-sync bootstrap update (`infra/BOOTSTRAP.md`), which re-pastes Step 3 anyway, rather than costing an operator step of its own — so it reaches the live account when that paste is run. **Lambda reserved concurrency was considered and dropped** 2026-08-23 rather than deferred: bounding parallelism does not bound spend when the usage plan already caps the month by request count, and the account runs one function. Reasoning is in `infra/variables.tf` and `docs/mcp-rag-design.md`.
- **Ungated docs-corpus publishing** — landed 2026-08-24: doc merges to main re-sync the RAG knowledge base automatically (`.github/workflows/corpus-sync.yml`, shared script with the gated deploy, narrow hand-created `bbb-mcp-corpus-sync` role; see `infra/BOOTSTRAP.md` section 3c). Ends the every-doc-PR manual deploy. Requires a one-time operator step: re-run the Step 3 CloudShell paste and add four repo variables (BOOTSTRAP "Update for the ungated corpus sync"). Until that is done the workflow fails loudly on its first run rather than skipping, so an unconfigured sync cannot be mistaken for a working one. The larger question behind it — whether code/infra deploys could also run ungated behind machine-checkable CI controls (plan assertions, lockfile provenance, resource-scope pinning) — is deliberately open; the gate stays on `mcp-deploy.yml` until such controls exist.
