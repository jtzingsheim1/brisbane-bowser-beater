...
- **MCP/infra hardening** — tracked in **issue #101**. The permissions-boundary change merged *and was applied* on 2026-08-23 (Step 3 CloudShell paste, then deploy run 8 with a successful `Terraform apply`): each role Terraform creates carries a ceiling the deploy role cannot create, edit, remove, or swap, and the boundaries are created by hand in the bootstrap so the pipeline cannot widen its own limit. `tests/infra-boundaries.test.ts` now holds the two boundary documents and the roles' inline Terraform policies to exact action parity in both directions (actions only, not resource scoping), pins each role's grants individually, and checks the deploy policy's boundary pinning and self-modification Deny. Verified by mutation: eighteen deliberate weakenings of `infra/`, each caught by a named test, plus two checks that a *stronger* policy still passes. Still open there: narrowing the workload boundary's Logs resource from `/aws/lambda/bbb-mcp-*` to the single granted log group (deliberately deferred to whichever change next needs a CloudShell re-paste, so it doesn't cost an extra operator step on its own), plus the watch items. **Lambda reserved concurrency was considered and dropped** 2026-08-23 rather than deferred: bounding parallelism does not bound spend when the usage plan already caps the month by request count, and the account runs one function. Reasoning is in `infra/variables.tf` and `docs/mcp-rag-design.md`.
